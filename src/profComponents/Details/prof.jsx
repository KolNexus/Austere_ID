...
const Prof = ({ kolId }) => {
  ...
  const filteredAssociations = useMemo(() => {
    return associations.filter(association =>
      association['Organization_Name'].toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const rows = filteredAssociations.map((association, index) => createAssociationData(
    `${association['Organization_Name']}-${index}`, // Ensure the key is unique by adding the index
    association['Organization_Name'],
    association['Organization_Type'],
    association['Board_Committee'],
    association['Position_Role'],
