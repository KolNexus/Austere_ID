...
const Body = ({ kolId, handleBackClick }) => {
  ...
  const rows = allEvents.map((event, index) => ({
    ...
    participationDate: handleNullValue(event['Event_Part_Date']),
    eventCountry: handleNullValue(event['Country']),
    conferenceName: handleNullValue(event['Event_Name']),
    sessionName:handleNullValue( event['Description']),
    sponsorName: handleNullValue(event['Sponsor_Name']),
    role: handleNullValue(event['Event_Role']),
    eventDetails: {
      startDate: handleNullValue(event['Event_Start_Date']),
      endDate: handleNullValue(event['Event_End_Date']),
      location: handleNullValue(event['Location']),
      city: handleNullValue(event['City']),
      state:handleNullValue( event['State']),
      abstract: handleNullValue(event['Abstracts']),
      sessionType: handleNullValue(event['Event_Type']),
    }
  }));
        ...
        const events = response.data.map(event => ({
          ...
          eventDetails: event.eventDetails.map(detail => ({
            ...
            eventDetails: {
              ...
              abstracts: handleNullValue(detail['eventDetails']?.['Abstracts']),
                          ...
                          <Collapse in={expandedDetailRow === index} timeout="auto" unmountOnExit>
                            <Box margin={1}>
                              ...
                              <Typography variant="body2">
                                ...
                                <strong>Abstract:</strong> {detail.eventDetails.abstracts}<br />
