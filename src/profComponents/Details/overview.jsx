...
const Overview = ({ kolDetails, setValue }) => {
  ...
  useEffect(() => {
    const fetchWeightages = async () => {
      try {
        const response = await apiClient.get(`/weightages`);
        const weightages = response.data.reduce((acc, item) => {
          acc[item['Legend'].toLowerCase().replace(/ /g, '')] = parseFloat(item['Max Value']);
          return acc;
        }, {});
        ...
        const medians = response.data.reduce((acc, item) => {
          acc[item['Legend'].toLowerCase().replace(/ /g, '')] = parseFloat(item['Median']);
          return acc;
        }, {});
  ...
  useEffect(() => {
    setBioText(kolDetails['Bio_Summary'] || '');
    setProgressValues({
      ...
      association: kolDetails['ASSOCIATION Count'],
      journal:kolDetails['JOURNAL Count'],
      industry:kolDetails['Industry Count'],
      press:kolDetails['Press']
    });
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
              <ProgressWithLabel
                value={progressValues.association}
                label="Association"
                targetValue={kolDetails['ASSOCIATION Count']}
                maxValue={maxValues.associationcount}
                medianValue={medianValues.associationcount}
                color="#7091E6"
              />
            </Grid>
            <Grid item xs={6}>
              <ProgressWithLabel
                value={progressValues.journal}
                label="Journal"
                targetValue={kolDetails['JOURNAL Count']}
                maxValue={maxValues.journalcount}
                medianValue={medianValues.journalcount}
                color="#7091E6"
              />
            </Grid>
            <Grid item xs={6}>
              <ProgressWithLabel
                value={progressValues.industry}
                label="Industry"
                targetValue={kolDetails['Industry Count']}
                maxValue={maxValues.industrycount}
                medianValue={medianValues.industrycount}
                color="#3D52A0"
              />
            </Grid>
            <Grid item xs={6}>
              <ProgressWithLabel
                value={progressValues.press}
                label="Press"
                targetValue={kolDetails['Press']}
                maxValue={maxValues.presscount}
                medianValue={medianValues.presscount}
                color="#3D52A0"
                onClick={() => setValue('press')}
              />
            </Grid>
          </Grid>
        </Grid>
